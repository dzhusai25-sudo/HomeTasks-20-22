...
describe("Parallel class", () => {
  ...
  it("корректно обрабатывает синхронные задачи", (done) => {
    const runner = new Parallel(2);
  ...
  it("обрабатывает ошибки в задачах", (done) => {
    const runner = new Parallel(2);

    runner
      .job((doneCb) => doneCb("success"))
      .job((doneCb) => doneCb(new Error("task failed")))
...
describe("debounce", () => {
  ...
  it("отменяет предыдущие таймеры при повторном вызове", () => {
    ...
    jest.advanceTimersByTime(60); // всего прошло 110 мс
    expect(mockFunc).toHaveBeenCalledTimes(1);
